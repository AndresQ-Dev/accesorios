...
function setBusy(next) {
  busy = next; loginButton.disabled = next; fileInput.disabled = next;
  previewButton.disabled = next || !fileInput.files?.[0]; confirmButton.disabled = next || !preview;
}
function clearPreview(focusFile = false) {
  preview = null; previewSummary.hidden = true; confirmButton.disabled = true; if (focusFile) fileInput.focus();
}
function returnToLogin(message) {
  csrfToken = null; clearPreview(); importPanel.hidden = true; loginPanel.hidden = false;
...
loginForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    csrfToken = payload.csrfToken; password.value = ''; loginPanel.hidden = true; importPanel.hidden = false;
